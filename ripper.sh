...
###
# curl -L https://emojipedia.org/au-kddi/$ver | sed -e '/emoji-grid/,$!d' -e '/lazymore/q' > ./$YEAR/assests
# cat $YEAR/assests | sed -e 's/<img.*src=/src=/g' -e 's/".data-srcset=.* alt=/" alt=/g' -e 's/".title.*>/"/g' | tr '\n' '~' | sed -e "s/<a.href=\"\/au-kddi\/$ver\//name=\"/g" -e 's/\/">~/" /g' | tr '~' '\n' > assests; mv ./assests > $YEAR/assests
